...
class tx_jfmulticontent_pi1 extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin
{
    ...
    public function main($content, $conf)
    {
        ...
        $this->pagerenderer = GeneralUtility::makeInstance(\tx_jfmulticontent_pagerenderer::class);
        ...
        if ($this->cObj->data['list_type'] == $this->extKey . '_pi1') {
            ...
            if ($this->lConf['style'] !== 'typoscript') {
                $this->lConf['columnOrder'] = $this->getFlexformData(
                    'general',
                    'columnOrder',
                    in_array($this->lConf['style'], ['2column', '3column', '4column', '5column'])
                );
                $this->lConf['column1'] = $this->getFlexformData(
                    'general',
                    'column1',
                    in_array($this->lConf['style'], ['2column', '3column', '4column', '5column'])
                );
                $this->lConf['column2'] = $this->getFlexformData(
                    'general',
                    'column2',
                    in_array($this->lConf['style'], ['2column', '3column', '4column', '5column'])
                );
                $this->lConf['column3'] = $this->getFlexformData(
                    'general',
                    'column3',
                    in_array($this->lConf['style'], ['3column', '4column', '5column'])
                );
                $this->lConf['column4'] = $this->getFlexformData(
                    'general',
                    'column4',
                    in_array($this->lConf['style'], ['4column', '5column'])
                );
                $this->lConf['column5'] = $this->getFlexformData(
                    'general',
                    'column5',
                    in_array($this->lConf['style'], ['5column'])
                );
                $this->lConf['equalize'] = $this->getFlexformData(
                    'general',
                    'equalize',
                    in_array($this->lConf['style'], ['1column', '2column', '3column', '4column', '5column'])
                );
                ...
                $this->lConf['tabHideTransitionduration'] = $this->getFlexformData(
                    'general',
                ...
                $this->lConf['tabShowTransitionduration'] = $this->getFlexformData(
                    'general',
                ...
                $this->lConf['accordionTransitionduration'] = $this->getFlexformData(
                    'general',
                ...
                $this->lConf['sliderTransitionduration'] = $this->getFlexformData(
                    'general',
                ...
                $this->lConf['slidedeckTransitionduration'] = $this->getFlexformData(
                    'general',
                    'slidedeckTransitionduration',
                    $debuglog
                );
                ...
                $this->lConf['delayDuration'] = $this->getFlexformData(
                    'general',
                    'delayDuration',
                    in_array($this->lConf['style'], ['slider', 'slidedeck', 'easyaccordion'])
                );
                $this->lConf['autoplayCycle'] = $this->getFlexformData(
                    'general',
                    'autoplayCycle',
                    ($this->lConf['style'] === 'slidedeck')
                );
            ...
            if ($this->conf['config.']['view'] === 'page') {
                    ...
                    if ($this->confArr['useOwnUserFuncForPages']) {
                        ...
                        if ($GLOBALS['TSFE']->sys_language_content) {
                            $res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
                                ...
                                'deleted=0 AND hidden=0 AND pid=' . (int)$page_ids[$a] . ' AND sys_language_uid=' . $GLOBALS['TSFE']->sys_language_content,
                                '',
                                '',
                                1
                            );
                        ...
                        if (!is_array($row)) {
                            $res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
                                '*',
                                'pages',
                                'deleted=0 AND hidden=0 AND uid=' . (int)$page_ids[$a],
                                '',
                                '',
                                1
                            );
                ...
                if ($this->conf['config.']['view'] === 'content') {
                    ...
                    for ($a = 0; $a < count($content_ids); $a++) {
                        ...
                        $GLOBALS['TSFE']->register['title'] = (strlen(
                            trim($this->titles[$a])
                        ) > 0 ? $this->titles[$a] : $row['header']);
                        if ($this->titles[$a] == '' || !isset($this->titles[$a])) {
                            $this->titles[$a] = $this->cObj->cObjGetSingle($view['title'], $view['title.']);
            ...
            case "tab" :
                {
                    ...
                    if ($this->conf['config.']['tabHideEffect'] === 'none') {
                        ...
                        if ($this->conf['config.']['tabHideTransition']) {
                            $fx[] = "easing:'" . (in_array(
                                    ...
                                    ["swing", "linear"]
                                ) ? "" : "ease{$this->conf['config.']['tabHideTransitiondir']}") . "{$this->conf['config.']['tabHideTransition']}'";
                        }
                    ...
                    if ($this->conf['config.']['tabShowEffect'] === 'none') {
                        ...
                        if ($this->conf['config.']['tabShowTransition']) {
                            $fx[] = "easing:'" . (in_array(
                                    $this->conf['config.']['tabShowTransition'],
                                    ["swing", "linear"]
                                ) ? "" : "ease{$this->conf['config.']['tabShowTransitiondir']}") . "{$this->conf['config.']['tabShowTransition']}'";
                        }
                    ...
                    $templateCode = trim(
                        $this->cObj->substituteSubpart($templateCode, '###TAB_SELECT_BY_HASH###', $tabSelector, 0)
                    );
                    ...
                    $templateCode = trim(
                        $this->cObj->substituteSubpart($templateCode, '###OPEN_EXTERNAL_LINK###', $openExtLink, 0)
                    );
            ...
            case "accordion" :
                {
                    ...
                    $markerArray["EASING"] = (in_array(
                        $this->conf['config.']['accordionTransition'],
                        ["swing", "linear"]
                    ) ? "" : "ease" . $this->conf['config.']['accordionTransitiondir'] . $this->conf['config.']['accordionTransition']);
                    $markerArray["TRANS_DURATION"] = (is_numeric(
                        $this->conf['config.']['accordionTransitionduration']
                    ) ? $this->conf['config.']['accordionTransitionduration'] : 1000);
            ...
            case "slider" :
                {
                    ...
                    if ($this->conf['config.']['sliderTransition']) {
                        $options[] = "easing: '" . (in_array(
                                $this->conf['config.']['sliderTransition'],
                                ["swing", "linear"]
                            ) ? "" : "ease{$this->conf['config.']['sliderTransitiondir']}") . "{$this->conf['config.']['sliderTransition']}'";
                    }
                    ...
                    if ($this->conf['config.']['sliderTheme']) {
                        ...
                        $this->pagerenderer->addCssFile(
                            GeneralUtility::slashJS($anythingSliderThemeFolder) . $this->conf['config.']['sliderTheme'] . '/style.css'
                        );
                    }
                    ...
                    if ($this->conf['config.']['sliderPanelFromHeader']) {
                        ...
                        $options[] = "navigationFormatter: function(i,p){ var str = '" . (GeneralUtility::slashJS(
                                $this->pi_getLL('slider_panel')
            ...
            case "slidedeck" :
                {
                    ...
                    if ($this->conf['config.']['slidedeckTransition']) {
                        $options['transition'] = "transition: '" . (in_array(
                                $this->conf['config.']['slidedeckTransition'],
                                ["swing", "linear"]
                            ) ? "" : "ease{$this->conf['config.']['slidedeckTransitiondir']}") . "{$this->conf['config.']['slidedeckTransition']}'";
                    }
            ...
            case "easyaccordion" :
                {
                    ...
                    if (!$templateCode = trim(
                        $this->cObj->getSubpart($this->templateFileJS, "###TEMPLATE_EASYACCORDION_JS###")
                    )) {
                    ...
                    $this->pagerenderer->addCssFile(
                        $this->confArr['easyAccordionSkinFolder'] . $this->conf['config.']['easyaccordionSkin'] . "/style.css"
                    );
                    $this->pagerenderer->addJS(trim($templateCode));
                    break;
                }
            case "booklet" :
                {
                    ...
                    if ($this->conf['config.']['bookletTransition']) {
                        $options['transition'] = "easing: '" . (in_array(
                                $this->conf['config.']['bookletTransition'],
                                ["swing", "linear"]
                            ) ? "" : "ease{$this->conf['config.']['bookletTransitiondir']}") . "{$this->conf['config.']['bookletTransition']}'";
                    }
    ...
    public function renderTemplate()
    {
        ...
        if ($this->conf['config.']['equalize']) {
            $markerArray["EQUALIZE_CLASS"] = ' ' . $this->cObj->stdWrap(
        ...
        for ($a = 0; $a < $this->contentCount; $a++) {
            ...
            if ($this->classes[$a] && isset($this->contentClass[$a]) && !preg_match("/class\=/i", $markerArray["ATTRIBUTE"])) {
                // wrap the class
                $markerArray["ATTRIBUTE"] .= $this->cObj->stdWrap(
                    $this->classes[$a],
                    ["wrap" => ' class="' . $this->contentClass[$a] . '"', "required" => 1]
                );
            }
            ...
            $markerArray["DEFAULT_QUOTE_TITLE"] = htmlspecialchars(
                $this->cObj->substituteMarkerArray($this->pi_getLL('default_quote_title_template'), $markerArray, '###|###', 0)
            );
            $markerArray["TAB_QUOTE_TITLE"] = htmlspecialchars(
                $this->cObj->substituteMarkerArray($this->pi_getLL('tab_quote_title_template'), $markerArray, '###|###', 0)
            );
            $markerArray["ACCORDION_QUOTE_TITLE"] = htmlspecialchars(
                $this->cObj->substituteMarkerArray($this->pi_getLL('accordion_quote_title_template'), $markerArray, '###|###', 0)
            );
            ...
            if (isset($this->conf['additionalContentMarkers'])) {
                ...
                if (count($additionalMarkers) > 0) {
                    foreach ($additionalMarkers as $additonalMarker) {
                        $markerArray[strtoupper($additonalMarker)] = $this->cObj->cObjGetSingle(
        ...
        if (isset($this->conf['additionalMarkers'])) {
            ...
            if (count($additionalMarkers) > 0) {
                foreach ($additionalMarkers as $additonalMarker) {
                    $additonalMarkerArray[strtoupper($additonalMarker)] = $this->cObj->cObjGetSingle(
                        $this->conf['additionalMarkerConf.'][$additonalMarker],
                        $this->conf['additionalMarkerConf.'][$additonalMarker . '.']
                    );
                }
...
if (defined('TYPO3_MODE') && $GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/jfmulticontent/pi1/class.tx_jfmulticontent_pi1.php']) {
    include_once($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/jfmulticontent/pi1/class.tx_jfmulticontent_pi1.php']);
}
