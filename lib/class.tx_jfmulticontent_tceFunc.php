    ...
    public function getCheckbox($PA, &$fObj)
    {
        ...
        if ($confArr['useSelectInsteadCheckbox']) {
            $PA['fieldConf']['config'] = [
                ...
                'items' => [
                    ['LLL:EXT:jfmulticontent/locallang_db.xml:tt_content.pi_flexform.from_ts', 2],
                    ['LLL:EXT:jfmulticontent/locallang_db.xml:tt_content.pi_flexform.yes', 1],
                    ['LLL:EXT:jfmulticontent/locallang_db.xml:tt_content.pi_flexform.no', 0],
                ],
            ];
        } else {
            $conf = $PA['fieldConf']['config'];
            $PA['itemFormElValue'] = (is_numeric(
                $PA['itemFormElValue']
            ) && $PA['itemFormElValue'] != 2 ? $PA['itemFormElValue'] : $conf['checked']);
            $PA['fieldConf']['config'] = [
                'type' => 'check',
            ];
        }
...
if (defined('TYPO3_MODE') && $GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/jfmulticontent/lib/class.tx_jfmulticontent_tceFunc.php']) {
    include_once($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/jfmulticontent/lib/class.tx_jfmulticontent_tceFunc.php']);
}
