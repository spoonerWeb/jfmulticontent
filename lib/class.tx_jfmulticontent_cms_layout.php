    ...
    public function getExtensionSummary($params, &$pObj)
    {
        if ($params['row']['list_type'] === 'jfmulticontent_pi1') {
            ...
            if (is_array($data) && $data['data']['general']['lDEF']['style']['vDEF']) {
                $result = sprintf(
                    $GLOBALS['LANG']->sL('LLL:EXT:jfmulticontent/locallang.xml:cms_layout.style'),
                    '<strong>' . $data['data']['general']['lDEF']['style']['vDEF'] . '</strong><br/>'
                );
            }
            if (!$result) {
...
if (defined('TYPO3_MODE') && $GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/jfmulticontent/lib/class.tx_jfmulticontent_cms_layout.php']) {
    include_once($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/jfmulticontent/lib/class.tx_jfmulticontent_cms_layout.php']);
}
